...
create definer = righthand@'%' view mh_filter_attendance as
...
create definer = righthand@`%` view mh_stat_attendance as
select `a`.`user_id`                                         AS `userid`,
       `a`.`clock_in`                                        AS `entry_time`,
       `b`.`clock_in`                                        AS `exit_time`,
       timestampdiff(MINUTE, `a`.`clock_in`, `b`.`clock_in`) AS `used_time`
from (`moya`.`mh_filter_attendance` `a`
         left join `moya`.`mh_exits` `b` on (((`a`.`user_id` = `b`.`user_id`) and
