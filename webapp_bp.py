...
from moya.driver_db import init_connect_db, add_newcard, is_rfid_bp, get_rfid_bp, get_userinfo_bp, set_exit_bp, set_attendance_bp, get_workingtimeWithUserid_bp, get_existence_bp
...
def index():
    return render_template('webapp.html', platform="반포도서관")
    ...
    try:
        ...
        if rst[0] != "not support this platform.":
            ...
            if rst[2] != None:
                ...
                name = get_userinfo_bp(db, userid, rfid_uid)
                if len(name) > 0:
                    #등록된 카드일 경우
                    data = get_existence_bp(db, userid)
                    value = int(data['value'])
                    if value == 1:
                        #등록은 됬으나 작업이력이 없는 경우
                        # print("등록은 됬으나 작업이력이 없는 경우")
                        rst.append("db true" if set_exit_bp(db, userid) else "db false")
                        rst.append({'name':'처음 온', 'total':'미적용 '})
                    else:
                        #등록도 됬고 작업이력이 있는 경우
                        # print("등록도 됬고 작업이력이 있는 경우")
                        info = get_workingtimeWithUserid_bp(db, userid)
                        rst.append("db true" if set_exit_bp(db, userid) else "db false")
                        rst.append(info[0]) 
                else:
                    #등록된 카드가 아닐 경우
        ...
        if rst[0] != "not support this platform.":
            ...
            if rst[2] != None:
                ...
                if len(userid) == 49:
                    ...
                    userid = rst[2]
                rfid_uid = rst[1]
                newmemeber = get_existence_bp(db, userid)
                name = get_userinfo_bp(db, userid, rfid_uid)
                if len(name) > 0:
                    #등록된 카드일 경우
                    data = get_existence_bp(db, userid)
                    ...
                    if value == 1:
                        #등록은 됬으나 작업이력이 없는 경우
                        # print("등록은 됬으나 작업이력이 없는 경우")
                        rst.append("db true" if set_attendance_bp(db, userid) else "db false")
...
def endpoint_rfid_read():
    try:
        ...
        if rst[0] != "not support the platform.":
            ...
            if rst[1] != None:
                rfid_uid = rst[1]
                # rfid_uid가 user_bp테이블에 있는지 확인하는 함수
                if is_rfid_bp(db, rfid_uid)['cnt'] == 0:
                    ...
                    uid = get_rfid_bp(db, rfid_uid)['id']
                    # 이미카드가 있는 경우
                    rfid_write(str(uid))
