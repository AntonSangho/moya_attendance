...
from moya.driver_db import init_connect_db, get_attendance, set_attendance, set_exit, get_userinfo, \
    ...
    set_exit_sw, \
    get_dayattendance_test, get_userdetail_test, set_modify_test, set_signup_test, get_adduserlist_test, get_userattendance_test,get_userselectdetail_test, get_RangeAttendance_test, get_moyaworkingtime_test,\
...
def aftermodify_test(username):
    ...
    if request.method == 'GET':
        ...
        print(selected_name)
        userworking = []
        for dbuser in get_moyaworkingtime_test(db, selected_name):
            user_time = {
                'time': {
                    'id': dbuser['id'],
                    'name': dbuser['name'],
...
def modify_sj(username):
    user = {'name': '관리자'}
    db = get_conn()
    userlist_info = []
    for dbuser in get_userselectdetail_sj(db, username):
        user_info = {
            'info': {
                'id': dbuser['id'],
                'name': dbuser['name'], 
