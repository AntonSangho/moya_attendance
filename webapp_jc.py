...
from moya.driver_db import init_connect_db, add_newcard, is_rfid, get_rfid, get_userinfo, set_exit, set_attendance, get_workingtimeWithUserid, get_existence
            ...
            if rst[2] != None:
                ...
                if len(name) > 0:
                    ...
                    if value == 1:
                        ...
                        info = get_workingtimeWithUserid(db, userid)
                        rst.append("db true" if set_exit(db, userid) else "db false")
                        rst.append(info[0]) 
                else:
                    #등록된 카드가 아닐 경우
        ...
        if rst[0] != "not support this platform.":
            ...
            if rst[2] != None:
                ...
                if len(userid) == 49:
                    ...
                    userid = rst[2]
                rfid_uid = rst[1]
                newmemeber = get_existence(db, userid)
                name = get_userinfo(db, userid, rfid_uid)
                if len(name) > 0:
                    #등록된 카드일 경우
                    data = get_existence(db, userid)
...
def endpoint_rfid_read():
    try:
        ...
        if rst[0] != "not support the platform.":
            ...
            if rst[1] != None:
                rfid_uid = rst[1]
                # rfid_uid가 user테이블에 있는지 확인하는 함수
                if is_rfid(db, rfid_uid)['cnt'] == 0:
                    # 새로운 카드 등록시 제천기적의도서관은 1번의 db 번호로 강제정의
